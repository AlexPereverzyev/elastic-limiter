...
module.exports.koa = function (key, options) {
    ...
    return (ctx, next) => {
        ...
        if (counter.isOpen()) {
            ...
            adaptRateLimit(counter, options, ctx)
            return next()
        }
    }
}
...
function adaptRateLimit(counter, options, ctx) {
...
function errorFor(counter, now, message) {
    ...
    err.retryAfter = (counter.recoversAfter ? counter.recoversAfter : counter.slice) - now + 1
    return err
}
